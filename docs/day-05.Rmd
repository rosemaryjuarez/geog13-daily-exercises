...
```{r}
covid %>% filter(date==max(date)) %>% arrange(-cases) %>% slice(1:5)
...
```{r}
covid %>% filter(date==max(date)) %>% group_by(state) %>% summarize(state_cases=sum(cases)) %>% arrange(-state_cases) %>% slice(1:5)
```
...
```{r}
covid %>% filter(date==max(date)) %>% filter(!cases==0, !county=="Unknown") %>% mutate(ratio=100*(deaths/cases)) %>% arrange(-ratio) %>% slice(1:5)
```
...
```{r}
covid %>% filter(date==max(date)) %>% filter(!cases==0, !county=="Unknown") %>% group_by(state) %>% summarize(state_cases=sum(cases), state_deaths=sum(deaths)) %>%
mutate(ratio=100*(state_deaths/state_cases)) %>% arrange(-ratio) %>% slice(1:5)
```
